...
// DB events store `date` as free text (e.g. "November 10–14, 2026"); the event card
// needs { day, month, year } to render the little date badge. Parse what we can,
// fall back to em-dashes if the text doesn't match a recognizable date pattern.
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from('staff').select('*').eq('category', 'current_executive').order('sort_order', { ascending: true }),
    ]).then(([upcomingRes, eventsRes, execRes]) => {
      if (cancelled) return
      setUpcomingEvents(upcomingRes.data || [])
      setSiteEvents(eventsRes.data || [])
  ...
  // Map DB event rows into the shape the event card renders
  const displayEvents = siteEvents.map(ev => ({
    tag: ev.tag,
  ...
  // Map DB staff rows into the shape the executive card renders
  const displayExecutives = executives
    .filter(s => s.name)
    .map(s => ({ name: s.name, role: s.role, img: s.photo_url }))
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-base)', transition: 'background 0.35s ease' }}>
      ...
      <section style={{ position: 'relative', minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', inset: 0 }}>
          <div style={{ position: 'absolute', inset: 0, backgroundImage: "url('https://cghds.run.edu.ng/assets/img/CGHDS_PHOTO_GALLERY/IMG_7105.JPG')", backgroundSize: 'cover', backgroundPosition: 'center', opacity: 'var(--img-opacity)' }} />
      ...
      <section className="section-py section-border">
        <C>
          ...
          {!dataLoaded ? (
            <div style={{ textAlign: 'center', padding: '48px 0', color: 'var(--text-faint)', fontFamily: 'Syne, sans-serif', fontSize: 13 }}>Loading events…</div>
          ) : displayEvents.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '48px 0', color: 'var(--text-faint)', fontFamily: 'Syne, sans-serif', fontSize: 13 }}>No events yet. Add one from the admin panel.</div>
          ) : (
      ...
      <section className="section-py section-border">
        <C>
          ...
          {!dataLoaded ? (
            <div style={{ textAlign: 'center', padding: '48px 0', color: 'var(--text-faint)', fontFamily: 'Syne, sans-serif', fontSize: 13 }}>Loading executives…</div>
          ) : displayExecutives.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '48px 0', color: 'var(--text-faint)', fontFamily: 'Syne, sans-serif', fontSize: 13 }}>No current executives added yet. Add them from the admin panel.</div>
          ) : (
            <div className="exec-grid">
              {displayExecutives.map((ex, i) => (
