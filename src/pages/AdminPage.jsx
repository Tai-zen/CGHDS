...
export default function Home() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (execRes.data?.length) setDbExecutives(execRes.data)
      setEventsLoaded(true)
    }).catch(() => { if (!cancelled) setEventsLoaded(true) })
      ...
      <section className="section-py section-border">
        <C>
          ...
          <div className="exec-grid">
            {(dbExecutives.length ? dbExecutives.map(s => ({ name: s.name, role: s.role, img: s.photo_url })) : executives).filter(ex => ex.name).map((ex, i) => (
